...
ParticleManager::initialise() {
  ...
  _shader = resourceManager.getShader(gero::asValue(ShaderIds::particles));

  auto geoDef = resourceManager.getGeometryDefinition(
    gero::asValue(GeometryIds::particles));
  ...
  gero::graphic::MakeGeometries::Vertices outVertices;
  outVertices.reserve(2048);

  //
  //

  gero::graphic::MakeGeometries::makeShpere(outVertices, 0, 0.5f);
  ...
  gero::graphic::MakeGeometries::makeShpere(outVertices, 1, 0.5f);
  for (const auto& vertex : outVertices)
    particlesVertices.push_back(vertex.position);
...
void
ParticleManager::setMatricesData(
