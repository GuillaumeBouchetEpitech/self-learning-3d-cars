...
SimplexNoise::SimplexNoise(float frequency /*= 1.0f*/,
  ...
  : _frequency(frequency), _amplitude(amplitude), _lacunarity(lacunarity),
...
uint8_t SimplexNoise::hash(int32_t i) const {
  return _perm[static_cast<uint8_t>(i)];
}
...
float SimplexNoise::noise(float x, float y) const {
  ...
  const float x1 =
    x0 - float(i1) + G2; // Offsets for middle corner in (x,y) unskewed coords
...
float SimplexNoise::noise(float x, float y, float z) const {
  ...
  float x2 =
    x0 - float(i2) + 2.0f * G3; // Offsets for third corner in (x,y,z) coords
