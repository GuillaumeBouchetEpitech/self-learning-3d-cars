...
ThirdPersonCamera::canActivate() const {
  ...
  // do not update the third person gero::graphic::Camera if not in a correct
  // state
...
ThirdPersonCamera::update(float elapsedTime) {
  ...
  if (canActivate()) {
    ...
    if (auto leaderData = leaderCar.leaderData()) {
      ...
      if (
        // do not update the third person gero::graphic::Camera if too close
        // from the target
        glm::distance(carOrigin, _eye) > 0.25f) {
        // simple lerp to setup the third person gero::graphic::Camera
