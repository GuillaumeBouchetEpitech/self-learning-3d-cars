...
FloorRenderer::initialise(const glm::vec3& center, const glm::vec3& size) {
  ...
  _shader =
    resourceManager.getShader(gero::asValue(ShaderIds::chessboardFloor));
...

  {
    ...
    _texture.allocateBlank(
      size, gero::graphic::Texture::Quality::smoothed,
      gero::graphic::Texture::Pattern::repeat, rawPixels);
  }
