...
AnimatedCircuitRenderer::initialise(
  ...
  _shaderWireframe =
    resourceManager.getShader(gero::asValue(ShaderIds::wireframes));
  _shaderCircuitLit =
    resourceManager.getShader(gero::asValue(ShaderIds::animatedCircuitGround));
  _shaderCircuit =
    resourceManager.getShader(gero::asValue(ShaderIds::animatedCircuitWalls));

  { // compute circuit skeleton wireframe geometry

    auto geoDef = resourceManager.getGeometryDefinition(
      gero::asValue(GeometryIds::wireframes));
...
AnimatedCircuitRenderer::update(float elapsedTime) {
  ...
  if (logic.isAccelerated) {
    ...
    // do we have a car to focus the gero::graphic::Camera on?
